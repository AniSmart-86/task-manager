...
const getTaskById = async(req,res)=>{
    try {
       ...
       res.json(task);
        ...
        res.status(500).json({message: "error getting task by id", error: error.message});
    }
};
    ...
    try {
       ...
       if(req.body.assignedTo){
        if(!Array.isArray(req.body.assignedTo)){
            return res.status(404).json({message: "assignedTo must be an array of users IDs"})
        }
...
const updateTaskChecklist = async (req, res) => {
  try {
    ...
    const completionCount = task.todoChecklists.filter(
      (item) => item.completed
    ).length;
...
const getUerDashboardData = async(req,res)=>{
    try {

        const userId = req.user._id; //loggin user only
        ...
        res.status(500).json({message: "error getting user dashboard data", error: error.message});
    }
}
